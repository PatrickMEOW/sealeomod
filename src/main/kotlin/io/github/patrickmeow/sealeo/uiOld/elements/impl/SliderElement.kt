...
class SliderElement(var x: Float, var y: Float, var setting: NumberSetting) : Element() {
    ...
    override fun draw(mouseX: Int, mouseY: Int) {
        ...
        if (draggingNow) {
            val normalizedProgress = (mouseX - (x + 20)) / 120f
            setting.value = (normalizedProgress * (setting.max.toFloat() - setting.min.toFloat()) + setting.min.toFloat())
                .coerceIn(setting.min.toFloat(), setting.max.toFloat())
        }

        RenderUtils.roundedRectangle(x - 110f, y - 8f, 280f, 30f, rectColor, 6f)
        RenderUtils.drawText(setting.name, x - 100f, y, -1, 1.2f)
        RenderUtils.roundedRectangle(x + 22.5f, y + 5f, 117.5f, 8f, sliderColor, 1.5f)

        val sliderProgress = (setting.value.toFloat() - setting.min.toFloat()) /
                (setting.max.toFloat() - setting.min.toFloat())
        RenderUtils.roundedRectangle(x + (sliderProgress * 120f) + 20, y + 4f, 10f, 10f, progressColor, 2f)
    ...
    override fun mouseClicked(mouseX: Int, mouseY: Int) {
        ...
        val sliderProgress = (setting.value.toFloat() - setting.min.toFloat()) /
                (setting.max.toFloat() - setting.min.toFloat())
        if (isMouseOver(mouseX, mouseY, x + (sliderProgress * 120f) + 10, y + 4f, 20f, 10f)) {
            draggingNow = true
        }
    }
